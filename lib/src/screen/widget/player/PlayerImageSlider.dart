...
class ImageSliderState extends State<ImageSlider> {
  ...
  final code = ResusableCode();
  int duration(PlayerState state) {
    if (state.isAlbum) {
      return state
          .artistSongList[state.currentAlbumIndex][state.index].duration;
    } else {
      return state.songlist[state.index].duration;
    }
  }
      ...
      builder: (context, state) {
        return Container(
          ...
          child: PageView.builder(
            ...
            itemCount: state.isAlbum
                ? state.artistSongList[state.currentAlbumIndex].length
            ...
            onPageChanged: (index) {
              StoreProvider.of<PlayerState>(context)
                  .dispatch(Player(index: index, isAlbum: true));
            },
          ),
        );
      },
  ...
  // navigate(context) {}
  bool imagePic(PlayerState state) {
    if (state.isAlbum) {
      if (state.artistSongList[state.currentAlbumIndex][state.index].albumArt !=
          null) {
        return true;
      }
    } else {
      if (state.songlist[state.index] != null) {
        return true;
      }
    }
    return false;
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      image: new DecorationImage(
                        ...
                        image: imagePic(state)
                            ? FileImage(
                                new File.fromUri(
                                  Uri.parse(state.isAlbum
                                      ? state
                                          .artistSongList[
                                              state.currentAlbumIndex][index]
                                          .albumArt
                                      : state.songlist[index].albumArt),
                                ),
                              )
                            : AssetImage(
